...
class DEQBase(nn.Module):
    def __init__(self, args):
        ...
        self.eval_f_thres = args.eval_f_thres if args.eval_f_thres > 0 else int(
            self.f_thres * args.eval_factor
        )
    ...
    def _log_convergence(self, info, name='FORWARD', color='yellow'):
        ...
        rel_lowest, abs_lowest = info['rel_lowest'].mean(
        ).item(), info['abs_lowest'].mean().item()
        nstep = info['nstep']
...
class DEQIndexing(DEQBase):
    def __init__(self, args):
        ...
        produce_grad = [
            backward_factory(
                grad_type=pg, tau=args.tau, sup_all=args.sup_all
        ...
        if args.ift:
            ...
            produce_grad[-1] = backward_factory(
                ...
                b_solver_kwargs=dict(
                    threshold=args.b_thres,
                    eps=args.b_eps,
                    stop_mode=args.b_stop_mode
                )
            )
        ...
        **kwargs
        ...
        with torch.no_grad():
            z_star, trajectory, info = self.f_solver(
                # To reuse previous coarse fixed points
                deq_func,
                x0=z_init,
                threshold=f_thres,
                eps=self.f_eps,
    ...
    def forward(
        ...
        if self.training:
            ...
            for z_pred, produce_grad in zip(trajectory, self.produce_grad):
                ...
                z_out += produce_grad(self, deq_func, z_pred)
            ...
            sradius = self._sradius(deq_func, z_star) if sradius_mode else torch.zeros(
                1, device=z_star.device
            )
    ...
    def forward(
        ...
        if self.training:
            ...
            for f_thres, produce_grad in zip(self.indexing, self.produce_grad):
                ...
                z_out += produce_grad(self, deq_func, z_star, writer=writer)
                # Add the gradient chain to the solver.
                z_star = z_out[-1]
